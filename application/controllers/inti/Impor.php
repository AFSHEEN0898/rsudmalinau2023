...
class Impor extends CI_Controller {
	...
	function impor_init() {
		...
		$this->db->query('
			...
			`bidang` varchar(128) DEFAULT NULL,
	...
	function push_data($t = null) {
		...
		foreach($sm->result() as $dat) {
			...
			if (!empty($nipp) and !empty($dat->nama)) {		
				...
				if (empty($c_unit->jml)) {
					$id_unit = $this->general_model->save_data('ref_unit', array('unit' => $dat->unit,'aktif' => 1,'level_unit' => '1'));
				} else {
				...
				$ess = !empty($ess_data[strtoupper($dat->eselon)])?$ess_data[strtoupper($dat->eselon)]:9;
