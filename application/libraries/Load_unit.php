...
class Load_unit {
	...
	function __construct() {
		...
		$this->in_app = $this->CI->general_model->datagrab(array(
			'tabel' => 'ref_aplikasi','where' => array('folder' => $this->CI->uri->segment(1))
		))->row()->nama_aplikasi;
	...
	function tabel_sub($p,$level_unit,$id) {
		...
		$sub_bid = $this->CI->general_model->datagrab(array('tabel' => $from, 'where' => array('id_par_unit' => $id),'select' => 'b.*,p.nama'));
		
		$no_urut = 1;
		foreach ($sub_bid->result() as $row) {
			$this->CI->general_model->save_data('ref_unit',array('urut' => $no_urut),'id_unit',$row->id_unit);
			$no_urut+=1;
		}
		...
		if ($sub_bid->num_rows() > 0) {
			foreach($sub_bid->result() as $row) {
				...
				$sub_bidd = $this->CI->general_model->datagrab(array('tabel' => 'ref_unit', 'where' => array('id_par_unit' => $row->id_unit)));
				...
				if (!empty($p['ubah'])) {
					$btn_tambah_sub = '<div class="btn-group">
						<a class="btn btn-xs btn-warning dropdown-toggle" data-toggle="dropdown" aria-expanded="true" href="#">
						<i class="fa fa-cog"></i>
						</a>
						...
						<li><a href="#" act="'.site_url($this->dir.'/unit/add_sub_unit/'.$row->level_unit.'/'.$row->id_unit).'" class="btn-edit dropdown-item"><i class="fa fa-plus-square fa-btn"></i> Sub Unit Kerja (Kantor Pusat / Kantor Cabang)</a></li>
						<li><a href="#" act="'.site_url($this->dir.'/unit/pindah_unit/'.in_de(array('id' => $row->id_unit))).'" class="btn-edit dropdown-item"><i class="fa fa-refresh fa-btn"></i> Pindahkan ke Bidang</a></li></div>
						</div>';
				}
	...
	function tabel_unit($o = null) {
		$s = $this->CI->general_model->get_param('multi_unit');
		$p = !empty($o) ? un_de($o) : array('ubah' => null,'link' => null);
		
		$root_bid = $this->CI->general_model->datagrab(array('tabel' => 'ref_unit','where' => array("id_par_unit = '' or id_par_unit IS NULL" => null),'order' => 'unit'));
		...
		if ($root_bid->num_rows() > 0) {
            $this->CI->table->set_template(array('table_open' => '<table class="table table-striped table-bordered table-condensed table-nonfluid" style="width: auto">'));
			...
			$this->CI->table->set_heading($head);
				...
				foreach ($root_bid->result() as $row) {
					...
					$sub_bid = $this->CI->general_model->datagrab(array('tabel' => 'ref_unit', 'where' => array('id_par_unit' => $row->id_unit)));
					if ($sub_bid->num_rows() == 0)  {
						$link2 = '<a href="#" class="btn btn-xs btn-danger btn-delete" act="'.site_url($this->dir.'/unit/removing/'.in_de(array('id' => $row->id_unit))).'" msg="Apakah unit <b>'.$row->kode_unit.' '.$row->unit.'</b> akan dihapus?"><i class="fa fa-trash"></i></a>';
					}
	...
	function add_unit($param) {
		...
		$combo_kepala = $this->CI->general_model->combo_box(array(
			'tabel' => $from,
			'key' => 'id_peg_jabatan',
		...
		$def = (!empty($o['id'])) ? $this->CI->general_model->datagrab(array('tabel' => 'ref_unit','where' => array('id_unit' => $o['id'])))->row():null;

		$whr_unit = !empty($def)?array('id_unit <>' => $def->id_unit):null;
		$cb_unit = $this->CI->general_model->combo_box(array(
			'tabel' => 'ref_unit',
			'key'=>'id_unit',
			'where' => $whr_unit,
			'val' => array('unit'),
		...
		$data['form_data'] = 
			...
			'<p>'.form_label('Telp').form_input('telp',@$def->telp,'class="form-control"').'</p>'.
	...
	function pindah_unit($p) {
		...
		$def = $this->CI->general_model->datagrab(array('tabel' => 'ref_unit','where' => array('id_unit' => $o['id'])))->row();	
		$combo_unit = $this->CI->general_model->combo_box(array('tabel' => 'ref_unit','key' => 'id_unit','val' => array('unit')));
	...
	function pindah($a) {
		...
		if ($a == "unker") {
			...
			); $this->CI->general_model->save_data('ref_bidang',$simpan);
			
			$this->CI->general_model->delete_data('ref_unit','id_unit',$id);
	...
	function add_sub_unit($level_unit,$id_par){
		...
		$par = $this->CI->general_model->datagrab(array('tabel' => 'ref_unit','where' => array('id_unit' => $id_par)))->row();
	...
	function saving() {
		...
		$this->CI->general_model->save_data('ref_unit',$simpan,'id_unit',$id);
	...
	function saving_sub(){
		...
		$this->CI->general_model->save_data('ref_unit',$simpan);
		$this->session->set_flashdata('ok', 'Data berhasil disimpan');
		redirect($this->dir.'/unit');
	...
	function removing($par){
		...
		$o = un_de($par);
		$this->CI->general_model->delete_data('ref_unit','id_unit',$o['id']);
	...
	function delete_unit(){
		...
		foreach($cek as $c) {
			$this->CI->general_model->delete_data('ref_unit','id_unit',$c);
		}
