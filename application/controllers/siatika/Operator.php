...
class Operator extends CI_Controller
{
    ...
    function list_data($search = null, $offset = null)
    {
        ...
        if ($this->indie and $this->unit) {
            $from = array(
                ...
                'ref_unit u' => array('u.id_unit = p.id_unit', 'left')
            );
            // $sel = 'p.nama,p.nip,p.username,p.id_pegawai,b.nama_bidang bidang, b.id_bidang,u.id_unit, u.unit';
            $sel = 'p.nama,p.nip,p.username,p.id_pegawai,u.id_unit, u.unit';
        } else {
            $from = 'peg_pegawai';
        ...
        if ($dtoperator->num_rows() > 0) {
            ...
            foreach ($dtoperator->result() as $row) {
                ...
                $link2 = ' <a href="' . site_url($this->dir . '/operator/reset_operator/' . $row->id_pegawai) . '" class="btn btn-xs btn-primary btn-reset" title="Reset Password" msg="Yakin untuk mereset password operator? <br>(Password standar adalah <b>qwerty</b>)"><i class="fa fa-redo"></i></a>';
    ...
    function add_operator($offs = null, $id = null)
    {
        ...
        $data['row']    = !empty($id) ? $this->general_model->datagrab(array(
            'tabel' => $from, 'where' => array('p.id_pegawai' => $id), 'select' => $sel
        ))->row() : null;
