 ...
 */
class ajax_pagination_informasi{
    ...
    var $anchor_class		= '';
    var $show_count         = true;
    var $link_func          = 'getDataInformasi';
    var $loading            = '.loading';
	...
	function create_links($jeda=3){
	  ...
	  //           $info = 'Showing : ' . $this->total_rows;
        ...
        function getDataInformasi(page_informasi){  
            $.ajax({
                method: "POST",
                url: "<?php echo $this->base_url; ?>"+page_informasi,
                data: { page_informasi: page_informasi },
                beforeSend: function(){
           ...
           // setTimeout(getDataInformasi(page), 9000);
           //return false;
        }

        $(document).ready(function() {
		  // run the first time; all subsequent calls will take care of themselves
		 var page_informasi = $('#page_informasi').val();
		  // setInterval(getDataInformasi(page_informasi), 90000000);
		  ...
		  // timer.stop();
			...
			var task = {
			    ...
			    callback: function (task) {
			    	getDataInformasi(page_informasi);
	...
	function getAJAXlink($count, $text='') {
		...
		return '<input type="hidden" id="page_informasi" value="'.$pageCount.'">';
