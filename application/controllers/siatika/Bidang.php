...
class Bidang extends CI_Controller
{
	...
	function tabel_unit($level, $o = null)
	{
		...
		$where = ($level == 1) ? array("(id_par_unit = '' or id_par_unit IS NULL or id_par_unit = 0)" => null) : array('id_par_unit' => $p['id_par']);

		if (isset($p['link']) or !isset($p['non'])) $where['aktif != 2'] = null;
		...
		if ($root_bid->num_rows() > 0) {
			...
			$this->table->set_template(array('table_open' => '<table class="table table-striped table-bordered table-condensed table-sm table-nonfluid">'));
			$this->table->set_empty("&nbsp;");

			$head = array();
			$head[] = array('data' => 'Nama / Kode Unit', 'colspan' => '6');
			...
			foreach ($root_bid->result() as $row) {
				...
				$unit_title = ($unit != NULL ? (strlen($unit) > 100 ? substr($unit, 0, 100) . ' ...' : $unit) . $kode : '[Tidak Ada Nama Unit Kerja]');

				$sel = !empty($p['link']) ? anchor(
					$p['link'] . in_de(array('link' => $p['link'], 'id' => $row->id_unit)),
					$unit_title,
					'class="link-normal btn-go" id="' . $row->id_unit . '" '
				) : $unit_title;
				...
				$rowsd[] = array('title' => $unit, 'data' => $sel . '&nbsp; &nbsp; ', 'colspan' => 7 - $level, 'class' => ($row->aktif == 1 or $row->aktif == null) ? null : 'nonaktif');

				if (count($rowsd) > 0) $this->table->add_row($rowsd);
			...
			if ($level == 1) {
				$this->table->set_template(array('table_open' => '<table class="table table-striped table-bordered table-condensed table-nonfluid table-sm">'));
				$this->table->set_empty("&nbsp;");
	...
	function list_unit($offset = null)
	{
		...
		if ($total_unit->num_rows() == 1) {
			...
			$this->tabel_unit(1, in_de(array('link' => $this->dir . '/Bidang/tabel_bidang/')));
	...
	function tabel_sub($level_bidang, $id)
	{
		...
		if ($sub_bid->num_rows() > 0) {
			...
			foreach ($sub_bid->result() as $row) {
				...
				$btn_down = ($r + 1 < $sub_bid->num_rows()) ? anchor($this->dir . '/Bidang/urutkan/' . in_de(array('unit' => $row->id_unit, 'id1' => $row->id_bidang, 'no1' => $row->urut, 'id2' => $awal_sub[$r + 1][0], 'no2' =>  $awal_sub[$r + 1][1])), '<i class="fa fa-arrow-down"></i>', 'class="btn btn-xs btn-info" style="margin-right: 5px" ') : '';
				...
				$cell_aksi[] = array('class' => 'text-center', 'data' => '<a href="#" class="btn-edit btn btn-xs btn-warning" act="' . site_url($this->dir . '/Bidang/add_bidang/' . in_de(array('id_par_bidang' => $row->id_par_bidang, 'id' => $row->id_bidang, 'id_unit' => $row->id_unit))) . '"><i class="fas fa-edit"></i></a>');
				...
				if ($sub_bidd->num_rows() == 0) {
					$cell_aksi[] = array('class' => 'text-center', 'data' => '<a href="#" class="btn-delete btn btn-xs btn-danger" act="' . site_url($this->dir . '/Bidang/removing/' . in_de(array('id' => $row->id_bidang, 'unit' => $row->id_unit))) . '" msg="Apakah Bidang <b>' . $row->kode_bidang . ' ' . $row->nama_bidang . '</b> akan dihapus?"><i class="fa fa-trash"></i></a>');
				} else {
					$cell_aksi[] =array('data' => '<button type="button" class="btn btn-xs btn-danger disabled"><i class="fas fa-trash"></i></button>','class' => 'text-center',);
				}
				...
				$btn_tambah_sub = '<div class="btn-group">
							<button type="button" class="btn btn-tool btn-xs"><i class="fas fa-cog"></i></button>
							<button type="button" class="btn btn-tool btn-xs dropdown-toggle dropdown-icon" data-toggle="dropdown" aria-expanded="false">
								<span class="sr-only">Toggle Dropdown</span>
								<div class="dropdown-menu" role="menu" style="">
									<a href="#" class="btn-edit dropdown-item" act="' . site_url($this->dir . '/Bidang/add_bidang/' . in_de($add_sub)) . '"><i class="fa fa-plus-square-o"></i> Tambahkan Sub Unit Organisasi</a>
									<a href="#" act="' . site_url($this->dir . '/Bidang/pindah_bidang/' . in_de(array('id' => $row->id_bidang, 'nama' => $row->nama_bidang))) . '" class="btn-edit dropdown-item"><i class="fa fa-refresh fa-btn"></i> Pindahkan ke Unit Kerja</a>
								</div>
							</button>
						</div>';
	...
	function tabel_bidang($o = null)
	{
		...
		if ($root_bid->num_rows() > 0) {
			// $this->table->set_template(array('table_open' => '<table class="table table-striped table-bordered table-condensed gap table-sm" >'));
			...
			foreach ($root_bid->result() as $row) {
				...
				$cell_aksi[] = array('class' => 'text-center', 'width' => '40', 'data' => '
						<a href="#" class="btn-edit btn btn-xs btn-warning" act="' . site_url($this->dir . '/Bidang/add_bidang/' . in_de(array('id' => $row->id_bidang, 'id_unit' => $row->id_unit))) . '"><i class="fas fa-edit"></i></a> &nbsp;');

				$sub_bid = $this->general_model->datagrab(array('tabel' => 'ref_bidang', 'where' => array('id_par_bidang' => $row->id_bidang)));
				if ($sub_bid->num_rows() == 0) {
					$cell_aksi[] = array('class' => 'text-center', 'width' => '35', 'data' => '<a href="#" class="btn-delete btn btn-xs btn-danger" act="' . site_url($this->dir . '/Bidang/removing/' . in_de(array('id' => $row->id_bidang, 'unit' => $row->id_unit))) . '" msg="Apakah Bidang <b>' . $row->kode_bidang . ' ' . $row->nama_bidang . '</b> akan dihapus?"><i class="fa fa-trash"></i></a>');
				} else {
					$cell_aksi[]	= array('data' => '<button type="button" class="btn btn-xs btn-danger disabled"><i class="fas fa-trash"></i></button>', 'class' => 'text-center',);
				}
			...
			$data['tabel'] =  '<div class="alert">Belum ada <strong>Unit Organisasi</strong> terdaftar </div>' . $btn_bid_root;
		}
		$data['extra_tombol'] = '<h4 class="box-title float-right">' . $unit_data->unit . '</h4>';
		$data['tombol'] = anchor($this->dir . '/Bidang/list_unit/', '<i class="fa fa-arrow-left"></i> Daftar Unit Kerja', 'class="btn btn-default"');
	...
	function add_bidang($param)
	{
		...
		if (!empty($o['id'])) {
			...
			if ($cek == 0) {
				...
				$form_par = '<div class="form-group">' . form_label('Unit Kerja') . form_dropdown('id_unit', $comb_unit, $o['id_unit'], 'class="combo-box" style="width: 100%"') . '</div>';
			} else {
		...
		if (!empty($o['id_par_bidang'])) {
			...
			if (empty($o['id'])) {
				...
				$form_par .= form_hidden('id_par_bidang', $o['id_par_bidang']) .
					'<div class="form-group">' . form_label('Atasan') . form_input('', $par->kode_bidang . ' ' . $par->nama_bidang, 'readonly="readonly" class="form-control"') . '</div>';
			} else {

				$this->dump_unit = array();
				$this->get_sub($o['id_unit'], $o['id']);

				$form_par .= '<div class="form-group">' . form_label('Unit Organisasi di Atasnya') . form_dropdown('id_par_bidang', $this->dump_unit, $o['id_par_bidang'], 'class="combo-box form-control " style="width: 100%"') . '</div>';
			}
		} else {
			...
			if (!empty($o['id'])) {
				...
				$form_par .= '<div class="form-group">' . form_label('Unit Organisasi di Atasnya') . form_dropdown('id_par_bidang', $this->dump_unit, null, 'class="combo-box form-control " style="width: 100%"') . '</div>';
			}
		}
		...
		$data['form_data'] =
			...
			'<div class="form-group">' . form_label('Kode') . form_input('kode_bidang', @$def->kode_bidang, 'class="form-control" required') . '</div>' .
			'<div class="form-group">' . form_label('Nama Unit Organisasi') . form_input('nama_bidang', @$def->nama_bidang, 'class="form-control" required') . '</div>';
