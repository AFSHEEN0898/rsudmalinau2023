...
class Sinkron extends CI_Controller {
	...
	function dasar($offs = null) {
		...
		for ($i = 0; $i < count($data); $i++) {
			...
			if (!empty($nipp) and !empty($data[$i]['nama'])) {
				...
				$this->urutan(array('id_pegawai' => $id_peg,'tabel' => 'peg_jabatan','kol' => 'id_peg_jabatan','order' => 'tmt_jabatan'));
				...
				if (!empty($gol_data->id_golru)) {
					...
					$this->urutan(array('id_pegawai' => $id_peg,'tabel' => 'peg_pangkat','kol' => 'id_peg_pangkat','order' => 'tmt_pangkat'));
	...
	function formal($offs = null) {
		...
		for ($i = 0; $i < count($data); $i++) {
			...
			if ($c_nip->jml > 0) {
				if (!empty($data[$i]['tahun_masuk']) and (!empty($data[$i]['jenjang']) or !empty($data[$i]['nama_jurusan']))) {
				...
				$this->urutan(array('id_peg' => $c_nip->id_pegawai,'tabel' => 'peg_formal','kol' => 'id_peg_formal','order' => 'tahun_selesai'));
	...
	function informal($offs = null) {
		...
		for ($i = 0; $i < count($data); $i++) {
			...
			if ($c_nip->jml > 0) {
				...
				$this->urutan(array('id_peg' => $c_nip->id_pegawai,'tabel' => 'peg_informal','kol' => 'id_peg_informal','order' => 'tanggal_selesai'));
	...
	function jabatan($offs = null) {
		...
		for ($i = 0; $i < count($data); $i++) {
			...
			if ($c_nip->jml > 0) {
				...
				$c_bid = $this->general_model->datagrab(array('tabel' => 'ref_bidang','where' => array('nama_bidang' => $data[$i]['nama_bidang'],'id_unit' => $id_unit),'select' => 'count(id_bidang) as jml,id_bidang'))->row();
				...
				if (empty($c_bid->jml)) {
					...
					$s_bid = array(
						'id_unit' => $id_unit,
						'nama_bidang' => $data[$i]['nama_bidang'],
						'urut' => $ur_bid->urutan+1);
					$id_bidang = $this->general_model->save_data('ref_bidang',$s_bid);
				} else {
					$id_bidang = $c_bid->id_bidang;
				...
				if (!empty($data[$i]['nama_jabatan']) and !empty($data[$i]['tmt_jabatan'])) {
				...
				$where_jab = array(
					...
					'id_unit' => $id_unit,
					'id_bidang' => $id_bidang,
					'tmt_jabatan' => $data[$i]['tmt_jabatan']);
				$cek_jab = $this->general_model->datagrab(array('tabel' => 'peg_jabatan','where' => $where_jab));
				...
				$this->urutan(array('id_peg' => $c_nip->id_pegawai,'tabel' => 'peg_jabatan','kol' => 'id_peg_jabatan','order' => 'tmt_jabatan'));
	...
	function pangkat($offs = null) {
		...
		for ($i = 0; $i < count($data); $i++) {
			...
			if ($c_nip->jml > 0) {
				...
				$this->urutan(array('id_peg' => $c_nip->id_pegawai,'tabel' => 'peg_pangkat','kol' => 'id_peg_pangkat','order' => 'tmt_pangkat'));
	...
	function kgb($offs = null) {
		...
		for ($i = 0; $i < count($data); $i++) {
			...
			if ($c_nip->jml > 0) {
				...
				$this->urutan(array('id_peg' => $c_nip->id_pegawai,'tabel' => 'peg_kgb','kol' => 'id_peg_kgb','order' => 'tmt_kgb'));
	...
	function diklatpim($offs = null) {
		...
		for ($i = 0; $i < count($data); $i++) {
			...
			if ($c_nip->jml > 0) {
				...
				$this->urutan(array('id_peg' => $c_nip->id_pegawai,'tabel' => 'peg_diklatpim','kol' => 'id_peg_diklatpim','order' => 'tanggal_selesai'));
				}
			} 
	...
	function belajar($offs = null) {
		...
		for ($i = 0; $i < count($data); $i++) {
			...
			if ($c_nip->jml > 0) {
				...
				$this->urutan(array('id_peg' => $c_nip->id_pegawai,'tabel' => 'peg_ijintugas_belajar','kol' => 'id_belajar','order' => 'tanggal_sk'));
	...
	function cuti($offs = null) {
		...
		for ($i = 0; $i < count($data); $i++) {
			...
			if ($c_nip->jml > 0) {
				...
				$this->urutan(array('id_peg' => $c_nip->id_pegawai,'tabel' => 'peg_cuti','kol' => 'id_peg_cuti','order' => 'tanggal_sk'));
	...
	function penghargaan($offs = null) {
		...
		for ($i = 0; $i < count($data); $i++) {
			...
			if ($c_nip->jml > 0) {
				...
				$this->urutan(array('id_peg' => $c_nip->id_pegawai,'tabel' => 'peg_penghargaan','kol' => 'id_peg_penghargaan','order' => 'tanggal_sk'));
	...
	function taspen($offs = null) {
		...
		for ($i = 0; $i < count($data); $i++) {
			...
			if ($c_nip->jml > 0) {
				...
				$this->urutan(array('id_peg' => $c_nip->id_pegawai,'tabel' => 'peg_taspen','kol' => 'id_peg_taspen','order' => 'tgl_berlaku'));
	...
	function hukdis($offs = null) {
		...
		for ($i = 0; $i < count($data); $i++) {
			...
			if ($c_nip->jml > 0) {
				...
				$c_hukdis = $this->general_model->datagrab(array('tabel' => 'ref_hukdis','where' => array('hukdis' => $data[$i]['nama_hukdis']),'select' => 'count(id_hukdis) as jml,id_hukdis'))->row();
				if ($c_hukdis->jml == 0) {
					$id_hukdis = $this->general_model->save_data('ref_hukdis', array('hukdis' => $data[$i]['nama_hukdis']));
				} else {
				...
				$where_hukdis = array(
					...
					'id_hukdis' => $id_hukdis,
					'tmt_hukdis' => $data[$i]['tmt_sk']);
				$cek_pkt = $this->general_model->datagrab(array('tabel' => 'peg_hukdis','where' => $where_hukdis));
				
				if ($cek_pkt->num_rows() == 0) {
					$this->general_model->save_data('peg_hukdis',$s_hukdis);
				} else {
					$this->general_model->save_data('peg_hukdis',$s_hukdis,'id_peg_hukdis',$cek_pkt->row()->id_peg_hukdis);
				}
				
				$this->urutan(array('id_peg' => $c_nip->id_pegawai,'tabel' => 'peg_hukdis','kol' => 'id_peg_hukdis','order' => 'tanggal_sk'));
		...
		for ($i = 0; $i < count($data); $i++) {
			...
			if ($c_nip->jml > 0) {
				...
				$cek_pensiun = $this->general_model->datagrab(array(
					...
					'where' => array('p.id_pegawai' => $c_nip->id_pegawai,'UPPER(j.nama_status)' => 'PENSIUN')
				))->row();
				...
				if ($cek_pensiun->jml == 0) {
					...
					if ($sv_pensiun->num_rows() > 0) {
						...
						$sts_pensiun = $this->general_model->datagrab(array('tabel' => 'ref_status_pegawai', 'where' => array('UPPER(nama_status)' => 'PENSIUN')))->row();
						if (!empty($sts_pensiun->nama_status)) {
						...
						$id_peg_jab_pensiun = $this->general_model->save_data('peg_jabatan',$simpan_pensiun);
						$this->urutan(array('id_peg' => $c_nip->id_pegawai,'tabel' => 'peg_jabatan','kol' => 'id_peg_jabatan','order' => 'tmt_jabatan'));
					...
					$id_peg_jab_pensiun = $cek_pensiun->id_peg_jabatan;
					
				}

				$s_pensiun = array(
					'id_pegawai' => $c_nip->id_pegawai,
					'id_peg_jabatan' => $id_peg_jab_pensiun,
		...
		for ($i = 0; $i < count($data); $i++) {
			...
			if ($c_nip->jml > 0) {
				...
				if (!empty($data[$i]['nip_penilai'])) {
					...
					if (empty($c_penilai->jml)) $id_penilai = $this->general_model->save_data('peg_pegawai', array('nip' => $data[$i]['nip_penilai']));
					else $id_penilai = $c_penilai->id_lokasi;
				}
				...
				$this->urutan(array('id_peg' => $c_nip->id_pegawai,'tabel' => 'peg_nilai','kol' => 'id_peg_nilai','order' => 'tanggal'));
	...
	function organisasi($offs = null) {
		...
		for ($i = 0; $i < count($data); $i++) {
			...
			if ($c_nip->jml > 0) {
				...
				$this->urutan(array('id_peg' => $c_nip->id_pegawai,'tabel' => 'peg_organisasi','kol' => 'id_peg_organisasi','order' => 'thn_mulai'));
	...
	function kawin($offs = null) {
		...
		for ($i = 0; $i < count($data); $i++) {
			...
			if ($c_nip->jml > 0 and !empty($data[$i]['nama_istri_suami'])) {
				...
				$this->urutan(array('id_peg' => $c_nip->id_pegawai,'tabel' => 'peg_perkawinan','kol' => 'id_perkawinan','order' => 'tanggal_kawin'));
	...
	function anak($offs = null) {
		...
		for ($i = 0; $i < count($data); $i++) {
			...
			if ($c_nip->jml > 0) {
				...
				$this->urutan(array('id_peg' => $c_nip->id_pegawai,'tabel' => 'peg_anak','kol' => 'id_anak','order' => 'tgl_lahir'));
	...
	function keluarga($offs = null) {
		...
		for ($i = 0; $i < count($data); $i++) {
			...
			if ($c_nip->jml > 0) {
				...
				$this->urutan(array('id_peg' => $c_nip->id_pegawai,'tabel' => 'peg_keluarga','kol' => 'id_peg_keluarga','order' => 'tgl_lahir'));
	...
	function urutan($par) {
		...
		$last = $this->general_model->datagrab(array('tabel'=>$par['tabel'],'where'=>array('id_pegawai'=>$par['id_peg']),'order'=>$par['order'],'select'=>$par['kol']))->row();
