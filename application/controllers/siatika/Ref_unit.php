...
class Ref_unit extends CI_Controller
{
	...
	function tabel_sub($p, $level_unit, $id)
	{
		...
		$from = array('ref_unit b' => '');

		$sub_bid = $this->general_model->datagrab(array('tabel' => $from, 'where' => array('id_par_unit' => $id), 'select' => 'b.*'));
		...
		if ($sub_bid->num_rows() > 0) {
			foreach ($sub_bid->result() as $row) {
				...
				$link1 = '<a href="#" class="btn btn-xs btn-warning btn-edit" act="' . site_url($this->dir . '/ref_unit/add_unit/' . in_de(array('level_unit' => $row->level_unit, 'id' => $row->id_unit))) . '"><i class="fa fa-pen"></i></a>';

				$sub_bidd = $this->general_model->datagrab(array('tabel' => 'ref_unit', 'where' => array('id_par_unit' => $row->id_unit)));
				if ($sub_bidd->num_rows() == 0) {
					$link2 = '<a href="#" class="btn btn-xs btn-danger btn-delete" act="' . site_url($this->dir . '/ref_unit/removing/' . in_de(array('id' => $row->id_unit))) . '" msg="Apakah unit <b>' . $row->kode_unit . ' ' . $row->unit . '</b> akan dihapus?"><i class="fa fa-trash"></i></a>';
				}
				...
				if (!empty($p['ubah'])) {
					$btn_tambah_sub = '<div class="btn-group">
						...
						<li><a href="#" act="' . site_url($this->dir . '/ref_unit/add_sub_unit/' . $row->level_unit . '/' . $row->id_unit) . '" class="btn-edit"><i class="fa fa-plus-square fa-btn"></i> Sub Unit Kerja (Kantor Pusat / Kantor Cabang)</a></li>
						<li><a href="#" act="' . site_url($this->dir . '/ref_unit/pindah_unit' . in_de(array('id' => $row->id_unit))) . '" class="btn-edit"><i class="fa fa-refresh fa-btn"></i> Pindahkan ke Bidang</a></li></ul>
						</div>';
				}
			...
			foreach ($root_bid->result() as $row) {
				...
				if (!empty($p['ubah'])) {
					...
					$btn_tambah_sub = '<div class="btn-group">
						...
						<ul class="dropdown-menu pull-right">
						<li class="dropdown-item"><a href="#" act="' . site_url($this->dir . '/ref_unit/add_sub_unit/' . $row->level_unit . '/' . $row->id_unit) . '" class="btn-edit"><i class="fa fa-plus-square fa-btn"></i> Sub Unit Kerja (Kantor Pusat / Kantor Cabang)</a></li>
						<li class="dropdown-item"><a href="#" act="' . site_url($this->dir . '/ref_unit/pindah_unit/' . in_de(array('id' => $row->id_unit))) . '" class="btn-edit"><i class="fa fa-redo fa-btn"></i> Pindahkan ke Bidang</a></li></ul>
						</div>';
				}
				$target = !empty($p['target']) ? 'target="' . $p['target'] . '"' : null;
	...
	function show_unit()
	{
		...
		$data['breadcrumb'] = array('' => $this->in_app, $this->dir . '/ref_unit' => 'Unit Kerja/SKPD');

		$data['title']		= 'Referensi Unit Kerja (Kantor Pusat / Kantor Cabang)';
		$data['content']	= 'umum/standard_view';

		$data['tabel'] =
			form_open($this->dir . '/ref_unit/delete_unit', 'id="form_delete"') .
		...
		$data['tombol'] =
			// ($s == 1) ? 
			anchor('#', '<i class="fa fa-plus-square"></i> &nbsp; Unit Kerja', 'class="btn btn-edit btn-success" act="' . site_url($this->dir . '/ref_unit/add_unit/' . in_de(array('level_unit' => '1'))) . '"')
			// : null
		;
		$data['content'] = "umum/standard_view";
	...
	function add_unit($param)
	{
		...
		$data['form_link'] = $this->dir . '/ref_unit/saving';
		...
		$data['form_data'] =
			...
			'<p>' . form_label('Kode Unit') . form_input('kode_unit', @$def->kode_unit, 'class="form-control " required ') . '</p>' .
			'<p>' . form_label('Nama Unit') . form_textarea('unit', @$def->unit, 'class="form-control" style="height: 60px" required') . '</p>' .
	...
	function pindah_unit($p)
	{
		...
		$data['form_link'] = $this->dir . '/ref_unit/pindah/unker';
	...
	function add_sub_unit($level_unit, $id_par)
	{
		...
		$data['form_link'] = $this->dir . '/ref_unit/saving_sub';
		...
		$data['form_data'] =
			...
			'</p>' .
			'<p>' . form_label('Kode Unit') . form_input('kode_unit', null, 'class="form-control" required') . '</p>' .
			'<p>' . form_label('Nama Unit') . form_textarea('unit', null, 'class="form-control" style="height: 60px" required') . '</p>' .
	...
	function removing($par)
	{
		...
		redirect($this->dir . '/ref_unit/show_unit');
	}
