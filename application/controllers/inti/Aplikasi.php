...
class Aplikasi extends CI_Controller {
	...
	function checking() {
		...
		foreach($fold as $fo) {
			$a=preg_replace('/\\\\/', '', $fo);
			//cek($a);
			if (!in_array($a,$app_in) and file_exists('./application/controllers/'.$fo.'/db.php')) {
		...
		//cek(count((array) $l_app));
		if (count((array) $l_app) > 0) {
			return $l_app;
		}
	...
	function load_setup($a) {
		$this->load->library('db_init');
		$a = $this->db_init->init_ref($a);
		//cek($a);
		...
		if ($s == 'root') {
			...
			foreach($query->result() as $row) {
				...
				$rows = array(
					...
					array('data' => $row->nama_aplikasi.' ('.$row->kode_aplikasi.')','class' => ($row->aktif == 1)?null:'text-muted'),
					array('data' => $row->deskripsi,'class' => ($row->aktif == 1)?null:'text-muted'));

					$s = null;
					if (file_exists('./application/controllers/'.$row->folder.'/db.php')) {
						$s = '<li>'.anchor(site_url($row->folder.'/db'),'Pemutakhiran Database', 'class="btn-db dropdown-item"').'</li>';
					} 
					...
					<div class="btn-group">
                      ...
                      <i class="fas fa-wrench"></i>
						...
						<div class="dropdown-menu dropdown-menu-right" role="menu" style="">
							<li>'.anchor('#','Ubah Redaksional', 'class="btn-edit dropdown-item" act="'.site_url('inti/Aplikasi/form_data/'.in_de(array('id' => $row->id_aplikasi,'status' => 'root'))).'"').'</li>
							<li>'.anchor('#',' Lepas &amp; Hapus Modul', 'class="btn-delete dropdown-item" act="'.site_url('inti/Aplikasi/delete_aplikasi/'.$row->id_aplikasi).'" msg="Apakah Anda ingin menghapus data <b>'.$row->kode_aplikasi.'</b>?"').'</li>'.$s. '
						</div>
					</div>
					';
                ...
                //           </ul>
                //         </div>';
					$rows[] = ($row->aktif == 1) ? anchor('inti/Aplikasi/saklar/off/'.$row->id_aplikasi,'<span class="btn btn-danger btn-xs"><i class="fas fa-power-off"></i></span>') : anchor('inti/Aplikasi/saklar/on/'.$row->id_aplikasi, '<span class="btn btn-default btn-xs"><i class="far fa-circle"></i></span>');
					if($trn or $naik){$rows[] = $trn.' '.$naik;}
					...
					foreach($ext->result() as $e) {
						...
						$ss = null;
						if (file_exists('./application/controllers/'.$row->folder.'/db.php')) {
						$ss = '<li>'.anchor(site_url($row->folder.'/db'),'Pemutakhiran Database', 'class="btn-db dropdown-item"').'</li>';
						} 
						
						$rowsd[] = '<div class="btn-group">
						<a class="btn btn-xs btn-warning btn-tool dropdown-toggle" data-toggle="dropdown" href="#" style="margin: 0 0 0 5px">
						<i class="fa fa-cog"></i>
						</a>
						<ul class="dropdown-menu float-right">
							<li>'.anchor('#','Ubah Redaksional', 'class="btn-edit dropdown-item" act="'.site_url('inti/Aplikasi/form_data/'.in_de(array('id' => $e->id_aplikasi,'status' => 'root'))).'"').'</li>
							<li>'.anchor('#','Lepas &amp; Hapus Modul', 'class="btn-delete dropdown-item" act="'.site_url('inti/Aplikasi/delete_aplikasi/'.$e->id_aplikasi).'" msg="Apakah Anda ingin menghapus data <b>'.$e->kode_aplikasi.'</b>?"').'</li>'.$ss.'
						</ul>
						</div>';
						
						$rowsd[] = ($e->aktif == 1) ?  anchor('inti/Aplikasi/saklar/off/'.$e->id_aplikasi,'<span class="btn btn-danger btn-xs"><i class="fas fa-power-off"></i></span>') :  anchor('inti/Aplikasi/saklar/on/'.$e->id_aplikasi,'<span class="btn btn-default btn-xs"><i class="far fa-circle"></i></span>');
			...
			$data['tabel'].= form_open('inti/Aplikasi/delete_aplikasi','id="form_delete"').
						...
						'<div class="table-responsive">' . $this->table->generate() . '</div>'.
	...
	function form_data($par) {
			...
			$data['form_data'] =
				...
				'<div class="form-group">'.form_label('Kode Aplikasi').form_input('kode_aplikasi',@$def->kode_aplikasi,'class="form-control" required').'</div>'.
				'<div class="form-group">'.form_label('Nama Aplikasi').form_input('nama_aplikasi',@$def->nama_aplikasi,'class="form-control" required'). '</div>'.
				'<div class="form-group">'.form_label('Deskripsi').form_textarea('deskripsi',@$def->deskripsi,'class="form-control" style="height: 75px" rows="3"'). '</div>'.
				'<div class="form-group">'.form_label('Warna').
				'<div class="input-group my-colorpicker2 colorpicker-element">
						<input name="warna" type="text" class="form-control" value="' . @$def->warna . '" data-original-title title />
						<div class="input-group-append">
							<span class="input-group-text"><i class="fas fa-square" style="color: ' . $warna . ' "></i></span>
						</div>
					</div>
				</div>
				...
				<div class="form-group">
						<label for="frm_file"></label>
						<div class="input-group">
							<div class="custom-file">
								<input type="file" class="custom-file-input" id="frm_file" name="icon_image">
