...
class Navi extends CI_Controller {
	...
	function tabel_sub($r,$lev,$a,$id) {
		...
		if ($sub_bid->num_rows() > 0) {
			foreach($sub_bid->result() as $row) {
				...
				$btn_down = ($n+1 < $sub_bid->num_rows()) ? anchor('inti/Navi/navurut/'.in_de(array('ref' => $r,'app' => $a,'id1' => $row->id_nav,'no1' => $row->urut,'id2' => @$awal[$n+1][0],'no2' => @$awal[$n+1][1])),'<i class="fas fa-arrow-down"></i>') : ' &nbsp; ';
				$btn_up = ($n > 0) ? anchor('inti/Navi/navurut/'.in_de(array('ref' => $r,'app' => $a,'id2' => $row->id_nav,'no2' => $row->urut,'id1' =>  !empty($urutkan[0])?$urutkan[0]:@$awal[0],'no1' => !empty($urutkan[1])?$urutkan[1]:@$awal[1])),'<i class="fas fa-arrow-up"></i>') : ' &nbsp; ';
				...
				$links[] = ($row->aktif == 1) ? anchor('inti/Navi/toggle/'.$r.'/'.$a.'/off/'.$row->id_nav,'<i class="fas fa-toggle-on"></i>') : anchor('inti/Navi/toggle/'.$r.'/'.$a.'/on/'.$row->id_nav,'<i class="fas fa-toggle-off"></i>'); 
				$links[] = array('class' => 'text-center','width' => '35','data' => $btn_down);
				$links[] = array('class' => 'text-center','width' => '35','data' => $btn_up);
				$tipe = ($row->tipe == 1) ? '<span class="label bg-yellow"><i class="fas fa-key"> </i></span>' : '<span class="label bg-purple"><i class="fas fa-bars"></i></span>';
				$icon = ($row->tipe == 1) ? 'star':(!empty($row->fa) ? $row->fa : 'circle');
				$kode = !empty($row->kode) ? '<b>'.$row->kode.'</b>' : null;
				$nama = !empty($row->judul) ? ' '.$row->judul : null;
				
				$btn_sub = ($row->tipe == 2 and $row->aktif == 1) ? '  &nbsp; <a href="#" class="btn-edit btn btn-xs btn-success" style="padding: 0 5px" title="Tambah SUB Modul ..." act="'.site_url('inti/Navi/form_data/'.in_de(array('ref' => $r,'id_aplikasi' => $a,'id_par' => $row->id_nav))). '"><i class="fas fa-arrow-circle-down"></i></a>' : null;
				$cl_cell = ($row->aktif == 1) ? ($row->tipe == 1?'text-orange':null):'text-muted';
				$tipe = ($row->aktif == 1) ? $tipe : '<span class="label label-default"><i class="fas fa-bars"></i></span>';
				
				$cell[] = array('data'=> '<i class="far fa-'.$icon.'"></i> &nbsp; '.$kode.$nama.$btn_sub,'colspan'=> 8-$lev,'class' => $cl_cell,'title' => $row->link);
	...
	function tabel($r,$a) {
		...
		$root_btn = anchor(
			'#',
			'<span class="btn btn-xs btn-success pull-right"><i class="fas fa-arrow-circle-down"></i> &nbsp; Root Modul</span>',
		...
		if ($nav->num_rows() > 0) {
			...
			$this->table->set_heading(array('data'=>'<input type="checkbox" name="cek_all" id="cek_all" class="cek-all"','class'=>'center','style'=>'width:30px'),
				...
				array('data' => 'Aksi', 'colspan' => 5,'class' => 'text-center'));
				$m = 0;
				foreach ($nav->result() as $row) {
					...
					$btn_down = ($m+1 < $nav->num_rows()) ? anchor('inti/Navi/navurut/'.in_de(array('ref' => $r,'app' => $a,'id1' => $row->id_nav,'no1' => $row->urut,'id2' => $awal[$m+1][0],'no2' =>  $awal[$m+1][1])),'<i class="fas fa-arrow-down"></i>') : ' &nbsp; ';
					$btn_up = ($m > 0) ? anchor('inti/Navi/navurut/'.in_de(array('ref' => $r,'app' => $a,'id2' => $row->id_nav,'no2' => $row->urut,'id1' =>  !empty($urutkan[0])?$urutkan[0]:@$awal[0],'no1' => !empty($urutkan[1])?$urutkan[1]:@$awal[1])),'<i class="fas fa-arrow-up"></i>') : ' &nbsp; ';
					$urutkan = array($row->id_nav,$row->urut);
					...
					$links[] = ($row->aktif == 1) ? anchor('inti/Navi/toggle/'.$r.'/'.$a.'/off/'.$row->id_nav,'<i class="fa fa-toggle-on"></i>') : anchor('inti/Navi/toggle/'.$r.'/'.$a.'/on/'.$row->id_nav,'<i class="fas fa-toggle-off"></i>'); 
					$links[] = array('class' => 'text-center','width' => '35','data' => $btn_down);
					$links[] = array('class' => 'text-center','width' => '35','data' => $btn_up);
					
					$tipe = ($row->tipe == 1) ? '<span class="text-orange"><i class="fas fa-key"> </i></span>' : '<span class="label bg-purple"><i class="fas fa-bars"></i></span>';
					$icon = ($row->tipe == 1) ? 'star':(!empty($row->fa) ? '<i class="fas fa-'.$row->fa. '"></i> ' : '<i class="far fa-circle"></i>');
					$kode = !empty($row->kode) ? '<b>'.$row->kode.'</b>' : null;
					$nama = !empty($row->judul) ? ' '.$row->judul : null;
					...
					$cell = array(
						...
						array('data'=> $icon.'&nbsp; '.$kode.$nama.$btn_sub,'colspan'=> 7,'class' => $cl_cell,'title' => $row->link));	
					$cell = array_merge_recursive($cell,$links);
	...
	function show_navi($par = null) {
		...
		$data['breadcrumb'] = array('' => 'Pengaturan', '#' => 'Navigasi');
		...
		'<div class="form-group">'.form_dropdown('pil',$app_combo,!empty($pil)?$pil:null, 'class="combo-box change-pil form-control"  style="width: 100%"').'</div>'.
			form_close();
			...
			$data['tabel'] = '
				<div class="row">
					<div class="col-lg-8">'.
						...
						anchor('#','<i class="fas fa-trash"></i> &nbsp; Hapus Yang Tercentang','class="btn btn-danger btn-delete-all btn-sm" style="display: none; margin-bottom: 10px"').
						$this->tabel($par['ref'],$pil).
						anchor('#','<i class="fas fa-trash"></i> &nbsp; Hapus Yang Tercentang','class="btn btn-danger btn-delete-all btn-sm" style="display: none; margin-top: -10px"').
						form_close().
					...
					form_open_multipart('inti/Navi/read_navi','id="form_impor"').
						...
						'<p>'.form_label(' &nbsp; ').form_button('btn-simpan','<i class="fas fa-download"></i> &nbsp; Impor','class="btn btn-import btn-success"').'</p></div>'.
						'<p>'.form_label('Ekspor Navigasi').'</p>'.anchor('inti/Navi/export_csv/'.in_de(array('app' => $pil,'ref' => $par['ref'])),'<i class="fas fa-upload"></i> &nbsp; Ekspor Navigasi','class="btn btn-warning"').'</p>'.
					form_close().
					'</div>';
	...
	function form_data($param) {
		...
		$form_par = '<div class="form-group">'.form_label('Aplikasi').form_input('',$apps->nama_aplikasi,'readonly="readonly" class="form-control"').'</div>';
		
		if (!empty($o['id_par'])) {
			$par =  $this->general_model->datagrab(array('tabel' => 'nav','where' => array('id_nav' => $o['id_par'])))->row();
			$form_par.= form_hidden('id_par_nav',$o['id_par']).
			'<div class="form-group">'.form_label('Navigasi Atas').form_input('',$par->kode.' '.$par->judul,'readonly="readonly" class="form-control"').'</div>';
		}
		...
		$data['form_data'] = 
			...
			'<div class="row">
				<div class="col-sm-8"><div class="form-group">'.form_label('Judul').form_input('judul',@$def->judul,'class="form-control judul-modul" required').'</div></div>'.
				'<div class="col-sm-4"><div class="form-group">'.form_label('Kode Kewenangan').form_input('kode',@$def->kode,'class="form-control kode-modul" '.$r_kode). '</div></div>
			</div>'.
			'<div class="form-group">'.form_label('Link').form_textarea('link',@$def->link,'class="form-control" style="height: 60px"').'</div>'.
			'<div class="form-group">'.form_label('Icon').form_input('fa',@$def->fa,'class="form-control"').'</div>'.
			form_hidden('id_nav',$id_nav). form_hidden('id_aplikasi',$o['id_aplikasi']).
			form_hidden('ref',$o['ref']);
