...
class Operator extends CI_Controller {
	...
	function list_data($search = null, $offset = null){
		...
		if($dtoperator->num_rows() > 0){
			...
			foreach($dtoperator->result() as $row){
				...
				$link1 = '<a href="#" act="'.site_url($this->dir.'/Operator/add_operator/'.(in_de($vse).'~'.$offset).'/'.$row->id_pegawai). '" class="btn btn-xs btn-warning btn-edit"><i class="fas fa-edit"></i></a>';
				$link2= ' <a href="'.site_url($this->dir.'/Operator/reset_operator/'.$row->id_pegawai). '" class="btn btn-xs btn-primary btn-reset" title="Reset Password" msg="Yakin untuk mereset password operator? <br>(Password standar adalah <b>qwerty</b>)"><i class="fas fa-redo-alt"></i></a>';
	...
	function add_operator($offs = null,$id = null) {
		...
		$data['title']	= !empty($id) ? 'Ubah Operator' : 'Tambah Operator';
		if ($this->indie and $this->unit) {
	...
	function save_foto() {
		...
		if (!empty($pasfoto)) {
			$pasfoto_lama = $this->general_model->datagrab(array('tabel' => 'peg_pegawai', 'select' => 'photo', 'where' => array('id_pegawai' => $id)))->row();
			//cek($this->db->last_query());
			$path_pasfoto = FCPATH . 'uploads/kepegawaian/pasfoto/' . $pasfoto_lama->photo;
			//cek($path_pasfoto); die();
			if (file_exists($path_pasfoto)) unlink($path_pasfoto);

			$config['upload_path'] = './uploads/kepegawaian/pasfoto/';
			...
			if (!$this->upload->do_upload('foto')) {
				...
				$konfigurasi = array(
					...
					'new_image' => './uploads/kepegawaian/pasfoto/',
