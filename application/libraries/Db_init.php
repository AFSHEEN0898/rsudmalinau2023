...
class Db_init {
	...
	function process_db($app, $tabel, $nav)
	{
		...
		$cek = $this->CI->general_model->datagrab(array(
			'tabel' => 'ref_aplikasi',
		...
		if ($cek->num_rows() > 0) {
			...
			$this->CI->general_model->save_data('ref_aplikasi', $app_plus, 'id_aplikasi', $cek->row()->id_aplikasi);
		} else {
			...
			$id_app = $this->CI->general_model->save_data('ref_aplikasi', $app_plus);
		}
		...
		if (!$this->CI->db->table_exists('log_db')) {
			...
			$this->CI->dbforge->add_key('id_log_db', TRUE);

			$this->CI->dbforge->create_table('log_db');
		}
		...
		foreach ($tabel as $tab => $c_tab) {
			$total += 1;
			if (!$this->CI->db->table_exists($tab)) {
				...
				$this->CI->dbforge->create_table($tab);

				foreach ($ex_fields as $c) {

					$this->CI->db->query('ALTER TABLE ' . $tab . ' ADD ' . $c[0] . ' ' . $c[1] . (!empty($c[2]) ? '(' . str_replace('.', ',', $c[2]) . ')' : null) . ' ' . (!empty($c[5]) ? 'DEFAULT ' . $c[5] : null));
				}
				...
				$this->CI->general_model->save_data('log_db', $si);
			} else {
				...
				foreach ($c_tab as $col => $c_col) {
					$column += 1;
					if (!$this->CI->db->field_exists($c_col[0], $tab)) {
						if (in_array($c_col[1], array('timestamp', 'decimal'))) {
							$this->CI->db->query('ALTER TABLE ' . $tab . ' ADD ' . $c_col[0] . ' 
								' . $c_col[1] .
							...
							$this->CI->dbforge->add_column($tab, $fields);
						}
		...
		foreach ($nav as $dat) {
			...
			if ($cek->jml == 0) {
				...
				$this->CI->general_model->save_data('nav', $simpan);
				$no += 1;
	...
	function data_db($db)
	{
		...
		foreach ($db as $k => $v) {
			...
			foreach ($v as $e => $val) {
				...
				if ($cek->num_rows() == 0) $this->CI->general_model->save_data($k, $smpn);
			}
		}
	}
		...
		if (!empty($par)) {
			switch ($par) {
				...
				case "db":
					...
					if (!$this->CI->general_model->check_tab('parameter')) {
						$this->CI->general_model->data_query("
					...
					if (!$this->CI->general_model->check_tab('nav')) {
						$this->CI->general_model->data_query("
					CREATE TABLE nav (
					...
					if (!$this->CI->general_model->check_tab('peg_pegawai')) {
						$this->CI->general_model->data_query("
					...
					if (!$this->CI->general_model->check_tab('pegawai_role')) {
						$this->CI->general_model->data_query("
					...
					if (!$this->CI->general_model->check_tab('ref_aplikasi')) {
						$this->CI->general_model->data_query("
					...
					if (!$this->CI->general_model->check_tab('ref_role')) {
						$this->CI->general_model->data_query("
					...
					if (!$this->CI->general_model->check_tab('ref_role_nav')) {
						$this->CI->general_model->data_query("
				...
				case "set":
					...
					for ($i = 0; $i < count($e); $i++) {
						...
						$this->CI->general_model->save_data('ref_aplikasi', array('aktif' => 1), 'folder', $e[$i]);
					}
