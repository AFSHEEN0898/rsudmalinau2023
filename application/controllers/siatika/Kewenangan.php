...
class Kewenangan extends CI_Controller
{
	...
	function list_kewenangan($offset = null)
	{
		...
		if ($dtkewenangan->num_rows() != 0) {
			...
			$this->table->set_template(array('table_open' => '<table class="table table-striped table-bordered table-condensed table-nonfluid" style="width: auto">'));
			...
			foreach ($dtkewenangan->result() as $row) {
				$link1 = anchor('#', '<i class="fas fa-edit"></i>', 'class="btn btn-xs btn-warning btn-edit" act="' . site_url($this->dir . '/Kewenangan/add_kewenangan/' . $row->id_role) . '"');
		...
		$data['tombol'] = anchor('#', '<i class="fa fa-plus-square"></i> &nbsp; Tambah Kewenangan', 'class="btn-edit btn btn-success" act="' . site_url($this->dir . '/Kewenangan/add_kewenangan') . '"');
		$data['tabel']	= $tabel;
	...
	function add_kewenangan($id_role = null)
	{
		...
		$where = ($this->app == 1) ?
			array('folder' => $this->dir) :
			array('id_aplikasi IN (' . $app_ac . ')' => null, 'id_par_aplikasi' => NULL);
		$data['aplikasi'] = $this->general_model->datagrab(array('tabel' => 'ref_aplikasi', 'where' => $where));
		...
		$data['link_save'] = $this->dir . '/Kewenangan/save_kewenangan';
		$data['app_data'] = $app_data;
		$data['title']	= !empty($id_role) ? 'Ubah Kewenangan' : 'Tambah Kewenangan';
		$data['def']	= !empty($id_role) ? $this->general_model->datagrab(array('tabel' => 'ref_role', 'where' => array('id_role' => $id_role)))->row() : null;

		$this->load->view($this->dir . '/kewenangan_form', $data);
	}
