 ...
 */
class ajax_pagination_pengumuman{
    ...
    var $anchor_class		= '';
    var $show_count         = true;
    var $link_func          = 'getDatapengumuman';
    var $loading            = '.loading';
	...
	function create_links($jeda=3){
	  ...
	  //           $info = 'Showing : ' . $this->total_rows;
        ...
        function getDatapengumuman(page_pengumuman){  
            $.ajax({
                method: "POST",
                url: "<?php echo $this->base_url; ?>"+page_pengumuman,
                data: { page_pengumuman: page_pengumuman },
                beforeSend: function(){
           ...
           // setTimeout(getDatapengumuman(page), 9000);
           //return false;
        }

        $(document).ready(function() {
		  // run the first time; all subsequent calls will take care of themselves
		 var page_pengumuman = $('#page_pengumuman').val();
		  // setInterval(getDatapengumuman(page_pengumuman), 90000000);
		  ...
		  // timer.stop();
			...
			var task = {
			    ...
			    callback: function (task) {
			    	getDatapengumuman(page_pengumuman);
	...
	function getAJAXlink($count, $text='') {
		...
		return '<input type="hidden" id="page_pengumuman" value="'.$pageCount.'">';
