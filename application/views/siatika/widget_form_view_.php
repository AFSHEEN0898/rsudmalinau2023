...
<?php echo 
	form_open_multipart('ids/list_ids/save_data/'.$code.'/'.$pos.'/'.$id,'id="form_widget"').
	...
	<div class="col-lg-5">
		<div class="box">
			...
			<div class="box-body">
				...
				<p><label>Durasi (Detik)</label>
					<?php echo form_input('durasi',@$def->durasi,'class="form-control" required') ?></p>
			</div>
			<div class="box-footer">
				<?php echo anchor('ids/list_ids/widget/'.$code,'<i class="fa fa-arrow-left"></i> Kembali','class="btn btn-default"') ?>
...
<script type="text/javascript">
	...
	function get_ids(posisi, target, post, target_ids, link_func, target_luar, konten){
		// concole.log(konten);
				$.ajax({
					url : "<?php echo site_url('ids/view/load_data_form/');?>/"+posisi+"/"+target+"/"+post+"/"+id_ids+"/"+target_ids+"/"+link_func+"/"+target_luar+"/"+konten+"/1",
					mthod : "POST",
	...
	$(document).ready(function() {
		...
		$('#pilih').change(function() {
			...
			$.ajax({
			url: '<?php echo site_url('ids/list_ids/load_form') ?>/'+$(this).val(),
			dataType: "json",
			...
			// $.ajax({
			// url: '<?php echo site_url('ids/list_ids/unit') ?>/'+$(this).val(),
			// success: function(msg){
		...
		<?php if (!empty($def)) { ?>
			...
			$('.box-form,.konten .box-body').html('<i class="fa fa-refresh fa-spin fa-btn"></i> Memuat ...');	
			$.ajax({
			url: '<?php echo site_url('ids/list_ids/load_form/'.$def->id_data.'/'.$def->id_konten) ?>',
			dataType: "json",
			success: function(msg){
				$('.box-form').html(msg.form);
				$('.konten').find('.box-title').html(msg.judul);
			...
			// $.ajax({
			// url: '<?php echo site_url('ids/list_ids/unit/'.$def->id_data.'/'.$def->id_konten) ?>',
			// success: function(msg){
			// 	$('.konten').find('.box-body').html(msg);
			// }
			// });
