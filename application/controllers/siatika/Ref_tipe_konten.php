...
class Ref_tipe_konten extends CI_Controller {
	var $dir = 'siatika/ref_tipe_konten';
	var $folder = 'siatika';
	...
	public function list_data($search=NULL, $offset=NULL){
		...
		if (!empty($search_key)) {
			$fcari = array(
				'a.tipe_konten' 		=> $search_key
			);
			$data['for_search'] = $fcari['a.tipe_konten'];
		} else if ($search) {
			$fcari=un_de($search);
			$data['for_search'] = $fcari['a.tipe_konten'];
		}
		$from = array('ids_tipe_konten a'=>'',
						'ref_aplikasi b'=>'b.id_aplikasi=a.id_aplikasi'
						);
