...
class ajax_pagination_video{
	...
	function create_links($jeda=3){
	  ...
	  //           $info = 'Showing : ' . $this->total_rows;
        ...
        function getDatavideo(page_video){  
            $.ajax({
                method: "POST",
                url: "<?php echo $this->base_url; ?>"+page_video,
                data: { page_video: page_video },
                beforeSend: function(){
           ...
           // setTimeout(getDatavideo(page), 9000);
           //return false;
        }

        $(document).ready(function() {
		  // run the first time; all subsequent calls will take care of themselves
		 var page_video = $('#page_video').val();
		  // setInterval(getDatavideo(page_video), 90000000);
	...
	function getAJAXlink($count, $text='') {
		...
		return '<input type="hidden" id="page_video" value="'.$pageCount.'">';
