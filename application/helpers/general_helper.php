...
function watermark($text,$configArray=array(),$filepath='uploads'){
    if(trim($text) == "")
        return "";
    $conf = array();
    $conf['background-color'] = isset($configArray['background-color']) ? $configArray['background-color'] : '#000000';
    $conf['color']            = isset($configArray['color'])            ? $configArray['color'] : '#ffffff';
    $conf['font-size']        = isset($configArray['font-size'])        ? $configArray['font-size'] : '14';
    $conf['font-file']        = isset($configArray['font-file'])        ? $configArray['font-file'] : 'bootstrap/css/bebas.otf';
...
// -- E: HELPER MILIK SIMAS --		
	...
	function get_stationer($id = null) {
		...
		if (isset($CI->config->config['otonom']) and $CI->config->config['otonom'] == TRUE) {
	...
	function get_nav($id_peg,$ref,$app,$id_par = null) {
		...
		foreach($nav->result() as $n) {
			...
			$icon = (!empty($n->fa)) ?  'fas fa-' . $n->fa . ' nav-icon':  'far fa-circle nav-icon';
			if ($ch->num_rows() > 0) {
				$j = check_uris($n->id_nav);
				$a = ($j == 'active') ? 'menu-open' :'';
				echo 
				'<li class="nav-item has-treeview '.$a.' ">
					<a href="#" class="nav-link ' . $j . '">
						<i class="' . $icon . '"></i> 
						<p>'.$n->judul. ' 
				...
				$j = uris($n->link);
				$j = !empty($j) ? 'class="nav-link active"': 'class="nav-link"';
				echo "<li class='nav-item' >".anchor($n->link,'<i class="'. $icon.'"></i> <p>'.$n->judul.'</p>', $j)."</li>";
			} 
	...
	function get_ref_nav($id_peg) {
		...
		foreach($nav->result() as $n) {
			...
			if ($ch->num_rows() > 0) {
				$j = check_uris($n->id_nav);
				$a = ($j == 'active') ? 'menu-open' : '';
				echo '<li class="nav-item has-treeview ' . $a . ' ">
					<a href="#" class="nav-link ' . $j . '"><i class="' . $icon . '"></i> <p>' . $n->judul . ' <i class="fas fa-angle-left right"></i></p> </a>
					<ul class="nav nav-treeview ">';
		...
		foreach($ro['aplikasi']  as $k => $v) {
			if ($v['direktori'] != 'referensi') {
				...
				if ($cek->num_rows() > 0) {
					$c = $cek->row();
					$j = check_uris($c->id_nav);
					// $k = $c->id_par_nav;
					//cek($j);
					$a = ($j == 'active') ? 'menu-open' : '';
					$icon = (!empty($c->fa)) ?  'fas fa-' . $c->fa . ' nav-icon' :  'far fa-circle nav-icon';
								
					echo 
					'<li class="nav-item has-treeview ' . $a . '">
						<a href="#" class="nav-link ' . $j . '">
							<i class="' . $icon . '"></i> <p>' . $v['nama_aplikasi'] . ' </p> <i class="fas fa-angle-left right"></i></a>
						</a>
						<ul class="nav nav-treeview">';
